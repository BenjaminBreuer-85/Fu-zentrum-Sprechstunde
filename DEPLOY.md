...
**Stand 17.09.2026: `sono.json` (neu) und `diagnosen.json` VOR dem Push von `app.html` hochladen.** Ab dem Commit „Sprechstundenbrief: Abschnitt ⑤ Sonographie" lädt `app.html` `sono.json` als elfte Datendatei; fehlt sie im Bucket, bricht die Live-App beim Laden mit DATEN-FEHLER ab.

**Stand 09.09.2026: FÜNF UPLOADS OFFEN — `opmethoden.json`, `opsteuerung.json`, `diagnosen.json`, `optexte.json`, `katalog2026.json`** `opsteuerung.json` am 09.09. nachmittags erneut geändert (Klartextnamen in `HDRG_REGELN`). `katalog2026.json` am 09.09. per Patch-Skript geändert (25 neue Kontextprozeduren, Flags bei 7 Kodes, `_KX` um 6 Kodes erweitert; Freigabe des Autors im Chat, Details in `abgleich-katalog-kontextprozeduren-2026-09-09.md`); die Master-Excel ist nachzuziehen, bevor die JSON das nächste Mal aus ihr erzeugt wird. (je: erst löschen, dann hochladen). Reihenfolge egal, alle vier Änderungen sind abwärtskompatibel (neue Felder werden von der heutigen `app.html` ignoriert). Die vier Uploads vom 30.08. (`optexte.json`, `opsteuerung.json`, `erloes2026.json`, `diagnosen.json`) hat der Autor am 30./31.08. bestätigt („alles geladen"); die zeichengenaue Bucket-Prüfung steht für alle noch aus.
...
|---|---|---|
| `sono.json` (neu), `diagnosen.json` | 17.09.2026 | **Upload offen, vor dem Push von `app.html`** — `sono.json` mit `GRUPPEN` und 20 `STRUKTUREN` (Normal- und Vorschlagstexte, Schritt 3r); `diagnosen.json` mit Feld `sono` je Diagnose (Vorauswahl der Strukturen). Freigabe des Autors 17.09.2026 („3r frei"). Selbsttest (11)/(12) prüft beide Dateien gegeneinander |
| `opsteuerung.json` | 09.09.2026 | **Upload offen** — neuer Eintrag `metallentfernung` (hdrg/drg leer, `empf: "amb"`, vier info-Hinweise). Damit lesen die drei Zweige Metallentfernung, Achillessehne und diabetischer Fuß ihre Werte aus `OP_STEUERUNG` statt aus fest verdrahtetem Code. Der erste Hinweis ist der `__fx`-Text aus dem alten Literal-Zweig, unverändert; den `kodier`-Text hat der Autor am 09.09.2026 in drei Hinweise zerlegt (manuelle Kodierung mit OPS 5-787.x und den ICD-Angaben Z47.0, T84.6, M84.1; Revision oder Wechsel als 5-78a.*; H-DRG-Trigger I20O mit den Zusatzeingriffen derselben Region), ohne Gliederungszeichen, Emojis und Zeilenumbrüche, die Bereiche „06–09" und „0b–0f" ausgeschrieben. Gegenprobe gegen den Originaltext aus der Git-Historie (vor c378f42): OPS-, ICD- und DRG-Kodes deckungsgleich, kein Kode verloren oder hinzugekommen. Ebenfalls offen: neuer Block `HDRG_REGELN` (Regeln je Hybrid-DRG nach Tabellen I20-V69/I20-V61, Ziel-DRGs je Kode) und `HDRG_RAHMEN` (fünf Rahmenbedingungen), Grundlage `auftrag-hybrid-regeln.md`; Lapidus-Hebel auf 5-783.0v + 5-784.0v korrigiert (5-784.0u ist keine Kontextprozedur), `hdrgTrigger` um 5-788.5e und 5-788.60 ergänzt, Warnhinweis nach Grouper-Prüfung des Autors; MTP-I-Hinweise ohne Spongiosa-Umweg; Ausschluss-Label bei Chevron, Chevron + Akin, Scarf, Youngswick, DMMO, Weil von „nur stationär" auf „stationär I20E/I20F, ambulant nur mit unvergüteten Zusatzkodes". Enthält die Chip-Umstellung der Code-Sitzung vom 07.09. (TMT-Varianten, Coalitio CN/TC, SMOT Valgus/Varus). Frühere offene Änderung vom 03.09. (`kontext[]` bei `arthrodese_osg`/`haglund_as_split` entfernt (reine stationäre Erlöshebel, keine Hybrid-Kontextprozeduren), stattdessen info-Hinweis „Erlöshebel stationär: …"; Kommentar zur Bedeutung von `kontext[]` im Dateikopf. Anlass: Block „Kontextprozeduren" erschien in der Live-App bei der USG-Arthrodese, obwohl der lokale Eintrag kein `kontext` hat — vor dem Upload Bucket-Kopie herunterladen und vergleichen (Verdacht: Bucket älter als lokal). Zugehöriger Code-Auftrag: `auftrag-kontext-nur-hybrid.md` |
| `diagnosen.json` | 07.09.2026 | **Upload offen** — neues optionales Feld `opText` (OP-abhängiger Diagnosetext) bei `usg_arthrose`, `lisfranc_arthrose`, `haglund`, `metatarsalgie`, `peronealsehnen`, `knick_senk`; `diagnoseText` bei `usg_arthrose` auf „Arthrose des unteren Sprunggelenkes" und bei `metatarsalgie` auf „Metatarsalgie" verallgemeinert; `bildBefunde` usg ohne „(Subtalargelenk)". Wirkt erst nach `auftrag-usg-diagnose-gelenke.md`, schadet vorher nicht |
| `optexte.json` | 07.09.2026 | **Upload offen** — drei neue OP-Bericht-Bausteine `tmt1_arthrodese` (medial, Platte + Zugschraube, ohne Korrektur), `tmt23_arthrodese` (dorsal), `tmt123_arthrodese` (zwei Zugänge); alte `tmt_arthrodese`/`tmt_arthrodese_2` bleiben bis zur Chip-Umstellung (`auftrag-chips-struktur.md`) |
| `opmethoden.json` | 07.09.2026 | **Upload offen** — Textkorrekturen nach Chip-Prüfung (`pruefung-chips-brieftexte-2026-09-07.md`): `subtalar_arthrodese` heißt „Subtalar-Arthrodese"; Beckenkamm bei allen Arthrodesen „ggf." im Brief und in der Aufklärung benannt; Lapidus mit fester Akin- und „ggf." subcapitaler Osteotomie; Peronealsehnennaht ohne Rekonstruktion; Strayer ohne Indikationszusatz; Haglund-Split mit Längsspaltung; Bandplastik mit Hamstring-/Plantarissehne; Coalitio mit Faszien-Fett-Lappen. Enthält weiterhin die Map `PATIENT_EINGRIFF_KB_MAP` vom 03.09.
...
Jede Änderung an einer der elf Bucket-Dateien wird hier eingetragen, mit Datum und Anlass, und beim Upload wieder ausgetragen. Wer eine Bucket-Datei ändert, ohne diesen Abschnitt zu ergänzen, hinterlässt genau die Unklarheit, die oben steht.

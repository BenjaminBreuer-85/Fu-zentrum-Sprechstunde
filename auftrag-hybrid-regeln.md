...
## Nachtrag 09.09.2026, sechster Teil: Vollzug bestätigt, ICD-Hinweise angehängt, Antwort zu MTP-I

1. **`metallentfernung`:** Die ICD-Hinweise hängen jetzt am Eintrag. Dabei habe ich den 1:1 übernommenen `kodier`-Text in drei `hinweise` vom Typ `info` zerlegt (manuelle Kodierung mit OPS 5-787.x und den ICD-Angaben Z47.0, T84.6, M84.1; Revision oder Wechsel der Osteosynthese als 5-78a.*; H-DRG-Trigger I20O mit den Zusatzeingriffen derselben Region). Alle Kodes stehen unverändert darin, die Bereichsangaben „06–09" und „0b–0f" sind als „bis" ausgeschrieben; Gliederungszeichen, Emojis und Zeilenumbrüche sind weg, wie im dritten Teil für die Freitexte vereinbart. Der erste Hinweis (`__fx`, Wert I20O) ist unangetastet. Der Originaltext bleibt in der Git-Historie von `app.html` (vor c378f42).

2. **MTP-I-Arthrodese plus 3 DMMO, I20D gegen I20E:** keine Katalogfrage, beides ist richtig. `mtp1_arthrodese.drg = I20E` ist die Ziel-DRG beim Ausweg über den Hebel Spongiosa (5-784.0v). Mit 5-788.54 gilt die Kontextregel aus `HDRG_REGELN.I20O_N`: `drg: {I20O: I20E, I20N: I20D}`, also aus I20N heraus nach I20D, ohne Spongiosa. Der Zweig hatte das fest kodiert, die Auswertung liefert es aus den Regeln. Nichts an den Daten ändern; bei der Umstellung der 28 Zweige ersetzt die Auswertung den festen Wert.

3. **Drei Zweige ohne Steuerungseintrag** (OSG-TEP-Wechsel, zwei Sprunggelenksfraktur-Zweige): bleiben im Vergleich stehen; die Einträge lege ich im Rahmen des 28-Zweige-Auftrags an, nachdem der Autor die Werte freigegeben hat. Bis dahin bleiben diese drei im Code.

4. **Toggle-Fälle** (mit oder ohne Spongiosa, ambulant oder stationär, mit oder ohne LCOT) sind keine Abweichungen, sondern genau die Fälle, die die Auswertung aus den Regeln erzeugt; sie brauchen keine Entscheidung.
